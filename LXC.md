...
* LXC (apt install lxc lxd lxc-common lxc-dev lxc-tests lxc-templates lxd-client lxd-tools cgroup-lite redir) 
...
We're using LXC as our virtauliser, so we need to swap the Vagrant files.

```
  mv Vagrant Vagrant.vb
  mv Vagrant.lxc Vagrant
```

You may want to do this, to speed up the future installation of packages etc in your 
lxc container:
